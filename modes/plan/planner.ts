...
export async function generatePlan(goal: string) {
  ...
  const hadWeb = !!process.env.FIRECRAWL_API_KEY;
  const model = wrapLanguageModel({
    model: getAgentModel(),
    middleware: extractJsonMiddleware(),
  });

  const tools = { ...readOnlyTools(executor), ...(hadWeb ? createWebTools(tracker) : {}) };
  ...
  const result = await generateText({
    ...
    system: PLAN_INSTRUCTIONS(config.codebasePath, hadWeb),
    prompt: `User Goal: \n${goal}`,
